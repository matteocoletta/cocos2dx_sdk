...
void Adjust2dx::setTestingMode(std::string baseUrl) {
#if (CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID)
    cocos2d::JniMethodInfo miSetTestingMode;

    if (!cocos2d::JniHelper::getStaticMethodInfo(miSetTestingMode, "com/adjust/sdk/AdjustFactory", "setTestingMode", "(Ljava/lang/String;)V")) {
        return;
    }

    jstring jBaseUrl = miSetTestingMode.env->NewStringUTF(baseUrl.c_str());

    miSetTestingMode.env->CallStaticVoidMethod(miSetTestingMode.classID, miSetTestingMode.methodID, jBaseUrl);

    miSetTestingMode.env->DeleteLocalRef(jBaseUrl);
#endif
}
