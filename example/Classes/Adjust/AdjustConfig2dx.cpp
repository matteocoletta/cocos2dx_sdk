...
void AdjustConfig2dx::setBasePath(std::string basePath) {
#if (CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID)
    if (config == NULL) {
        return;
    }

    cocos2d::JniMethodInfo miSetBasePath;

    if (!cocos2d::JniHelper::getMethodInfo(miSetBasePath, "com/adjust/sdk/AdjustConfig", "setBasePath", "(Ljava/lang/String;)V")) {
        return;
    }

    jstring jBasePath = miSetBasePath.env->NewStringUTF(basePath.c_str());

    miSetBasePath.env->CallVoidMethod(config, miSetBasePath.methodID, jBasePath);

    miSetBasePath.env->DeleteLocalRef(jBasePath);
#endif
}
